...
class ServiceCoordinator(Node):
    ...
    def publish_initial_pose(self):
        ...
        if self.init_pose_count >= 10:
            # Stop the timer after 10 publications
        ...
        # Set initial orientation (identity quaternion)
    ...
    def stop_robot(self):
        ...
        # Create a counter for the stop command publications
        self.stop_count = 0
        
        def publish_stop():
            self.stop_count += 1
            self.cmd_vel_pub.publish(stop_cmd)
            if self.stop_count >= 10:  # Stop after 10 publications (1 second)
                self.stop_timer.cancel()
        
        # Create timer that publishes stop commands at 10Hz
        self.stop_timer = self.create_timer(0.1, publish_stop)
    
    def create_kitchen_pose(self) -> PoseStamped:
        """Create kitchen position pose."""
        kitchen = PoseStamped()
        kitchen.header.frame_id = 'map'
        kitchen.pose.position.x = 17.9397  # Adjust as needed
        kitchen.pose.position.y = 8.45744  # Adjust as needed
    ...
    def calculate_approach_pose(self, target_pose: PoseStamped, distance: float) -> PoseStamped:
        """Calculate approach pose at a given distance from target."""
    ...
    def customer_pose_callback(self, msg: PoseStamped):
        ...
        # If navigating to customer, update goal
    ...
    def start_recording_with_delay(self):
        """Start recording with a small delay to ensure subscriber is ready."""
    ...
    def deliver_to_customer(self):
        """Navigate back to customer for delivery."""
        if self.current_customer_pose is None:
    ...
    def send_navigation_goal(self, goal_msg: NavigateToPose.Goal, 
        ...
        """Send navigation goal and setup callbacks."""
        ...
        # Setup callback for goal response
    ...
    def goal_result_callback(self, future, completion_callback: Callable[[bool], None]):
        """Handle navigation result and ensure robot stops."""
    ...
    def order_taking_callback(self, success: bool):
        """Handle completion of navigation to customer."""
        if success:
    ...
    def kitchen_arrival_callback(self, success: bool):
        """Handle completion of navigation to kitchen."""
        if success:
    ...
    def delivery_complete_callback(self, success: bool):
        """Handle completion of delivery navigation."""
        if success:
    ...
    def home_arrival_callback(self, success: bool):
        """Handle completion of home navigation."""
        if success:
            self.current_state = self.states['IDLE']
            self.current_customer_pose = None
